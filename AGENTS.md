...
**SPA (GitHub Pages):** `deploy-gh-pages.yml` fetches the custom domain via `gh api repos/<repo>/pages --jq '.cname'` and passes it as `SITE_HOST` for `SSR=false pnpm build`. `vite.config.ts` reads `process.env.SITE_HOST?.trim()` (falls back to `localhost`) and bakes `https://${SITE_HOST}` into the static `sitemap.xml` / `robots.txt` — each emitted by its own build plugin (`app/vite/plugins/{sitemap,robots}.ts`, via `this.emitFile`) **only in SPA mode** (gated on `SSR === 'false'`). The deploy workflow also writes `build/client/CNAME` from the same value (GH Pages requires `/CNAME`); there is no committed `public/CNAME` to drift.
